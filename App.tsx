...
import {
  Alert, KeyboardAvoidingView, Platform, Pressable, SafeAreaView, ScrollView,
  StyleSheet, Text, TextInput, View,
} from 'react-native';
import { drawTeams, setTarget, setWinner, setsToWin } from './src/game';
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
          ...
          {!teams ? <>
            ...
            <View style={styles.teams}>{(['A', 'B'] as TeamKey[]).map(team => <View key={team} style={styles.teamCard}><Text style={styles.teamCardTitle}>TIME {team}</Text>{(team === 'A' ? teams.teamA : teams.teamB).map(player => <Text key={player.id} style={styles.member}>•  {player.name}</Text>)}</View>)}</View>
