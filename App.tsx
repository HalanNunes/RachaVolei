      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
          ...
          {!teams ? <>
            ...
            <Text style={styles.dragHint}>Arraste um jogador na direção do outro time</Text>
            <View style={styles.teams}>{(['A', 'B'] as TeamKey[]).map(team => {
              const teamPlayers = team === 'A' ? teams.teamA : teams.teamB;
              return <View key={team} style={styles.teamCard}><Text style={styles.teamCardTitle}>TIME {team}</Text>{teamPlayers.length ? teamPlayers.map(player => <DraggablePlayer key={player.id} player={player} team={team} onMove={() => movePlayer(player, team)} onRemove={() => removeCourtPlayer(player)} />) : <Text style={styles.emptyTeam}>Time sem jogadores</Text>}</View>;
            })}</View>
